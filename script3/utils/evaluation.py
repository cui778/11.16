...
def evaluate_model(model,
                   dataloader,
                   criterion_node,  # 应该传入 KLDivLoss（与训练一致）
                   device,
    ...
    # --------- 定位子集：has_defect==1 的累积量 ---------
    n_samples_has_defect = 0
    topk_hit_has_defect = {k: 0 for k in topk_values}
    mrr_sum_has_defect = 0.0
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            if isinstance(logits, tuple):
                ...
                logits_has_defect = logits[1] if len(logits) > 1 else None
            else:
                logits_node = logits
                logits_has_defect = None

            if logits_has_defect is not None and 'has_defect' in batch:
                has_defect_batch = batch['has_defect'].long().to(device)
                cls_loss = torch.nn.functional.cross_entropy(logits_has_defect, has_defect_batch)
                # cross_entropy 默认 reduction='mean'，这里要按样本数加权汇总
                cls_loss_sum += float(cls_loss.item()) * int(has_defect_batch.numel())
                cls_pred = torch.argmax(logits_has_defect, dim=1)
                cls_correct += int((cls_pred == has_defect_batch).sum().item())
                cls_samples += int(has_defect_batch.numel())
            ...
            if is_localization:
                ...
                has_defect_mask = None
                if 'has_defect' in batch:
                    has_defect_mask = (batch['has_defect'].long().to(device) > 0)
                ...
                has_defect_valid = None
                if has_defect_mask is not None:
                    has_defect_valid = has_defect_mask[valid_mask]
                ...
                if soft_labels_valid is not None:
                    ...
                    # 回退：硬标签 + CE（兼容旧数据）
                    scores_masked = scores_valid.masked_fill(~cand_mask_valid, -1e9)
                    loss = criterion_node(scores_masked, y_idx_valid)
                ...
                if has_defect_valid is not None:
                    subset = has_defect_valid.view(-1).bool()
                    if subset.any():
                        n_samples_has_defect += int(subset.sum().item())
                        mrr_sum_has_defect += float(torch.sum(1.0 / ranks[subset].float()).item())
                ...
                for k in topk_values:
                    ...
                    if has_defect_valid is not None:
                        subset = has_defect_valid.view(-1).bool()
                        if subset.any():
                            topk_hit_has_defect[k] += int((ranks[subset] <= k).sum().item())
    ...
    if n_samples > 0:
        ...
        if n_samples_has_defect > 0:
            metrics['mrr_has_defect'] = (mrr_sum_has_defect / n_samples_has_defect)
            for k in topk_values:
                metrics[f'topk_recall_{k}_has_defect'] = topk_hit_has_defect[k] / n_samples_has_defect
        else:
    ...
    if cls_samples > 0:
        metrics['cls_loss'] = cls_loss_sum / max(1, cls_samples)
        metrics['cls_acc'] = cls_correct / max(1, cls_samples)
