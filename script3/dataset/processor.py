...
class DefectDetectionDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 allowed_defect_types: Optional[List[str]] = None):
        ...
        self.allowed_defect_types = None if allowed_defect_types is None else {str(x).upper() for x in allowed_defect_types}
    ...
    def _build_dataset(self, node_timeseries_file, adjacency_matrix_file,
        ...
        if self.allowed_defect_types is not None and 'defect_type' in self.defect_matrix.columns:
            _before = len(self.defect_matrix)
            _dtype = self.defect_matrix['defect_type'].astype(str).str.upper()
            self.defect_matrix = self.defect_matrix[
                _dtype.isin(self.allowed_defect_types) | _dtype.eq('BASELINE')
            ].copy()
            print(f"[OK] defect type filter: {_before} -> {len(self.defect_matrix)} ({sorted(self.allowed_defect_types)})")
        self.defect_info = {}
        ...
        if feature_names is None:
            # 自动选择可用的特征
            # 优先使用残差特征，如果不存在则使用原始特征
            candidate_features = [
                ('depth_residual', 'flow_residual', 'BODf_residual', 'NH4_residual', 'DO_residual'),
                ('depth', 'flow', 'BODf', 'NH4', 'DO'),
            ]
        ...
        print(f"[OK] 使用特征 ({len(self.feature_cols)}): {self.feature_cols}")
        ...
        for scenario_id, defect_info in self.defect_info.items():
            ...
            for i in range(0, num_windows, self.window_stride):
                # 直接切片提取窗口,无需任何查询
                window_features = feature_array[i:i + self.sequence_length]
                ...
                if self.candidate_mask_np is not None and self.candidate_mask_np[defect_node_idx] < 0.5:
                    logger.warning(
                ...
                if defect_info.get('defect_type', 'BASELINE') != 'BASELINE':
                    if label_mode == 'always_on':
                        ...
                        has_defect = 1 if overlap_ratio > 0 else 0
                else:
                    has_defect = 0
                ...
                signal_score = 0.0
                if has_defect > 0:
                    residual_slice = window_features[:, defect_node_idx, self.residual_feature_indices]
                    signal_score = float(np.mean(np.abs(residual_slice)))

                if defect_info.get('defect_type', 'BASELINE') == 'BASELINE':
                    loc_enabled = False
                elif label_mode == 'always_on':
                    loc_enabled = bool(self.always_on_force_target)
                else:
                    loc_enabled = bool(overlap_ratio >= self.overlap_threshold)

                if self.signal_threshold > 0:
                    loc_enabled = loc_enabled and (signal_score >= self.signal_threshold)
                ...
                sample = {
                    ...
                    'has_defect': has_defect,  # 0. 新增：图级二分类标签
                    ...
                    'target_node_idx': target_node_idx,  # ✅ 单标签定位目标（全局节点索引）；无定位则 -1
                    'soft_label': soft_label,
                    'signal_score': signal_score,
                    'loc_enabled': int(loc_enabled),
    ...
    def _print_statistics(self):
        ...
        has_defect_list = [int(s.get('has_defect', 0)) for s in self.samples]
        n_has_defect = sum(has_defect_list)
        n_no_defect = len(has_defect_list) - n_has_defect
        ...
        print(f"\n窗口级标签统计:")
        print(f"  has_defect=1: {n_has_defect} ({(n_has_defect / max(1, len(self.samples)) * 100):.1f}%)")
        print(f"  has_defect=0: {n_no_defect} ({(n_no_defect / max(1, len(self.samples)) * 100):.1f}%)")
        print(
            f"  定位启用(target_node_idx>=0): {n_loc_enabled} ({(n_loc_enabled / max(1, len(self.samples)) * 100):.1f}%)")

        if self.signal_threshold > 0:
            print(f"  信号阈值: {self.signal_threshold:.4f}")
...
def create_dataloaders(node_timeseries_file: str,
    ...
    if (split_mode or "").strip().lower() == "node_holdout":
