...
def simulate_and_extract_timeseries_fixed_time(base_inp_path, defect_csv_path,
    ...
    if baseline_node_data is not None and len(baseline_node_data) > 0:
        ...
        if baseline_link_data is not None and len(baseline_link_data) > 0:
            all_link_data.append(baseline_link_data)
        ...
        extraction_stats['total_link_records'] += 0 if baseline_link_data is None else len(baseline_link_data)
    ...
    for idx, row in df_defects.iterrows():
        ...
        if time_gated:
            # 时间门控参数解析
            start_hour = row.get("start_hour", 0)  # 默认从0小时开始
            duration_h = row.get("duration_h", 24)  # 默认持续24小时

            # 修复时间基准：匹配SWMM仿真时间（2025年）
            start_time = datetime(2025, 1, 1, start_hour, 0, 0)  # 修复：使用正确的年份
        ...
        try:
            ...
            if node_data is not None and len(node_data) > 0:
                ...
                extraction_stats['total_link_records'] += 0 if link_data is None else len(link_data)
    ...
    if len(all_node_data) > 0:
        ...
        df_links_all = None
        if len(all_link_data) > 0:
            df_links_all = pd.concat(all_link_data, ignore_index=True)
            link_output_parquet = os.path.join(output_dir, "link_timeseries.parquet")
            df_links_all.to_parquet(link_output_parquet, index=False, compression='gzip')
            logger.info(f"✓ 管道数据已保存: {link_output_parquet}")
        ...
        return df_nodes_all, df_links_all, df_summary
    else:
