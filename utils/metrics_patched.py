...
def compute_khop_hit_from_indices(pred_node_idx: int,
    ...
    dist = compute_graph_distance(adj_matrix, pred_node_idx, true_node_idx, max_hops=k_hops)
...
def compute_distance_distribution_from_indices(pred_node_idx: int,
    ...
    dist = compute_graph_distance(adj_matrix, pred_node_idx, true_node_idx, max_hops=max_hops)
    if dist <= cap:
        return {int(dist): 1}
    return {f'>{cap}': 1}
