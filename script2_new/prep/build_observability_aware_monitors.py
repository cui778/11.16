...
# -*- coding: utf-8 -*-
"""
Build an observability-aware monitor layout under a fixed monitor budget.

Design goal:
- keep the monitor budget fixed (default N=25)
- maximize candidate-node observability
- explicitly reduce the number of far candidates (>2 hops from any monitor)

This script does not use labels or train/test results. It only uses:
- candidate node set
- graph shortest-hop structure
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build observability-aware N25 monitor layout.")
    ...
    for _ in range(args.n):
        ...
        for j, node in enumerate(all_nodes):
            ...
            hop_reduction = float(best_hops[best_hops < INF_HOP].mean() - new_hops.mean()) if (best_hops < INF_HOP).any() else 0.0
    ...
    out = {
        ...
        "objective": {
            "primary": "maximize number of candidate nodes within 2 hops of at least one monitor",
            "secondary": "increase direct coverage and reduce mean candidate-monitor hop",
            "hop_utility": {"0": 6.0, "1": 4.0, "2": 3.0, "3": 0.5, ">3": 0.0},
        },
