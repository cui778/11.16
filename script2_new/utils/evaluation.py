...
def _scene_score_from_probs(probs: np.ndarray, mode: str = "topk_mean", topk: int = 5) -> float:
    """Aggregate window-level active probabilities into one scene-level score."""
    arr = np.asarray(probs, dtype=np.float32)
    if arr.size == 0:
        return 0.0
    mode = str(mode or "topk_mean").lower()
    if mode == "max":
...
def evaluate_model(model,
    ...
    active_false_positive = 0
    active_actual_negative = 0
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            if logits_active is not None and ('active_label' in batch or 'has_defect' in batch):
                ...
                active_false_positive += int(((active_pred == 1) & (active_target_batch == 0)).sum().item())
                active_actual_negative += int((active_target_batch == 0).sum().item())
            else:
                ...
                if active_target_batch is not None and active_pred is not None and 'scenario_id' in batch and 'window_start_time' in batch:
                    ...
                    for i in range(int(scores.size(0))):
                        ...
                        scenario_windows.setdefault(sc_key, []).append({
                            ...
                            'active_prob': float(active_prob[i].item()) if active_prob is not None else float(active_pred[i].item()),
                        })
    ...
    if active_samples > 0:
        ...
        active_precision = _safe_div(active_true_positive, active_pred_positive)
        active_recall = _safe_div(active_true_positive, active_actual_positive)
        active_f1 = _f1(active_precision, active_recall)
        normal_window_fpr = _safe_div(active_false_positive, active_actual_negative)
        ...
        metrics['active_f1'] = active_f1
        metrics['window_precision'] = active_precision
        metrics['window_recall'] = active_recall
        metrics['window_f1'] = active_f1
        metrics['normal_window_fpr'] = normal_window_fpr
        ...
        metrics['active_f1'] = 0.0
        metrics['window_precision'] = 0.0
        metrics['window_recall'] = 0.0
        metrics['window_f1'] = 0.0
        metrics['normal_window_fpr'] = 0.0
    ...
    n_events = 0
    scene_threshold = (
        float(scene_active_threshold)
        if scene_active_threshold is not None
        else float(active_prob_threshold) if active_prob_threshold is not None
        else 0.5
    )
    scene_tp = scene_fp = scene_tn = scene_fn = 0
    scene_alarm_times = []
    scene_detection = {}
    ...
    for sc_key, windows in scenario_windows.items():
        ...
        active_probs = np.asarray([float(w.get('active_prob', 0.0)) for w in ordered], dtype=np.float32)
        scene_score = _scene_score_from_probs(active_probs, mode=scene_score_mode, topk=scene_topk)
        scene_score_topk_mean = _scene_score_from_probs(active_probs, mode="topk_mean", topk=scene_topk)
        scene_score_max = _scene_score_from_probs(active_probs, mode="max", topk=scene_topk)
        true_scene_has_defect = any(int(w.get('true_active', 0)) == 1 for w in ordered)
        pred_scene_has_defect = bool(scene_score >= scene_threshold)
        if true_scene_has_defect and pred_scene_has_defect:
            scene_tp += 1
        elif true_scene_has_defect and not pred_scene_has_defect:
            scene_fn += 1
        elif (not true_scene_has_defect) and pred_scene_has_defect:
            scene_fp += 1
        else:
            scene_tn += 1
        ...
        first_alarm = None
        scene_alarm_time = float('nan')
        if active_truth:
            t0_scene = np.datetime64(active_truth[0]['window_start_time'])
            first_alarm = next((
                w for w in ordered
                if np.datetime64(w['window_start_time']) >= t0_scene
                and float(w.get('active_prob', 0.0)) >= scene_threshold
            ), None)
        if active_truth and first_alarm is not None:
            ta_scene = np.datetime64(first_alarm['window_start_time'])
            scene_alarm_time = float((ta_scene - t0_scene) / np.timedelta64(1, 'h'))
            scene_alarm_times.append(scene_alarm_time)
        scene_detection[sc_key] = {
            'n_windows': int(len(ordered)),
            'true_has_defect': int(true_scene_has_defect),
            'pred_has_defect': int(pred_scene_has_defect),
            'scene_score': float(scene_score),
            'scene_score_topk_mean': float(scene_score_topk_mean),
            'scene_score_max': float(scene_score_max),
            'threshold': float(scene_threshold),
            'scene_alarm_time_h': float(scene_alarm_time),
        }
    ...
    scene_count = scene_tp + scene_fp + scene_tn + scene_fn
    scene_precision = _safe_div(scene_tp, scene_tp + scene_fp)
    scene_recall = _safe_div(scene_tp, scene_tp + scene_fn)
    metrics['scene_has_defect_acc'] = _safe_div(scene_tp + scene_tn, scene_count)
    metrics['scene_precision'] = scene_precision
    metrics['scene_recall'] = scene_recall
    metrics['scene_f1'] = _f1(scene_precision, scene_recall)
    metrics['scene_fpr'] = _safe_div(scene_fp, scene_fp + scene_tn)
    metrics['scene_alarm_time_mean'] = float(np.mean(scene_alarm_times)) if scene_alarm_times else float('nan')
    metrics['scene_alarm_time_count'] = int(len(scene_alarm_times))
    metrics['scene_threshold'] = float(scene_threshold)
    metrics['scene_score_mode'] = str(scene_score_mode)
    metrics['scene_topk'] = int(scene_topk)
    metrics['scene_count'] = int(scene_count)
    metrics['scene_defect_count'] = int(scene_tp + scene_fn)
    metrics['scene_normal_count'] = int(scene_fp + scene_tn)
    metrics['scene_detection'] = scene_detection
