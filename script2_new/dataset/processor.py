...
NORMAL_DEFECT_TYPES = {"BASELINE", "NORMAL", "NO_DEFECT", "NONE", "NAN", ""}


def _normalize_defect_type(value) -> str:
    text = "" if value is None else str(value).strip().upper()
    return "BASELINE" if text in NORMAL_DEFECT_TYPES else text
...
class DefectDetectionDataset(Dataset):
    ...
    def _build_dataset(self, node_timeseries_file, adjacency_matrix_file,
        ...
        if node_timeseries_file.endswith('.parquet'):
            try:
                if os.environ.get("SWMM_PARQUET_SINGLE_THREAD") == "1":
                    import pyarrow.parquet as pq
                    table = pq.read_table(
                        node_timeseries_file,
                        use_threads=False,
                        memory_map=False,
                    )
                    self.node_data = table.to_pandas(use_threads=False)
                    print("  [OK] SWMM_PARQUET_SINGLE_THREAD=1")
                else:
        ...
        for _, row in self.defect_matrix.iterrows():
            ...
            defect_type = _normalize_defect_type(row.get('defect_type', 'BASELINE'))
            self.defect_info[scenario_id] = {
                'defect_type': defect_type,
                'node_id': '' if defect_type == 'BASELINE' else str(row.get('node_id', '')),
                'link_id': '' if defect_type == 'BASELINE' else str(row.get('link_id', '')),
        ...
        # Multi-baseline normal scenes can appear in the time-series file without
        # rows in the defect matrix; treat them as no-defect scenarios.
        for scenario_id in sorted(scenario_ids_in_data - set(self.defect_info.keys())):
            self.defect_info[int(scenario_id)] = {
...
def create_dataloaders(node_timeseries_file: str,
                       ...
                       persistent_train_sample_ratio: float = 1.0,
    ...
    persistent_ratio = float(persistent_train_sample_ratio or 1.0)
    if persistent_ratio < 1.0 and train_indices.size > 0:
        keep = []
        persistent = []
        for idx in train_indices:
            sid = int(dataset.samples[int(idx)].get("scenario_id", -1))
            if sid >= 900000:
                persistent.append(int(idx))
            else:
                keep.append(int(idx))
        if persistent:
            n_keep = max(1, int(round(len(persistent) * max(persistent_ratio, 0.0))))
            n_keep = min(n_keep, len(persistent))
            selected_persistent = np.random.choice(np.array(persistent, dtype=int), n_keep, replace=False)
            train_indices = np.concatenate([np.array(keep, dtype=int), selected_persistent])
            np.random.shuffle(train_indices)
            print(
                f"\n[Persistent训练下采样] ratio={persistent_ratio:.3f}, "
                f"persistent窗口: {len(persistent)} -> {n_keep}, 训练样本数: {len(train_indices)}"
            )
