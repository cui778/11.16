...
def build_config_snapshot(config, split_diag: dict) -> dict:
    ...
    snapshot = {
        ...
        'active_prob_threshold': getattr(config, 'active_prob_threshold', None),
        'scene_active_threshold': getattr(config, 'scene_active_threshold', None),
        'scene_score_mode': getattr(config, 'scene_score_mode', 'topk_mean'),
...
def main():
    ...
    _parser.add_argument("--active-threshold", type=float, default=None, help="window-level active probability threshold")
    _parser.add_argument("--scene-threshold", type=float, default=None, help="scene-level has_defect threshold")
    _parser.add_argument("--scene-score-mode", type=str, default=None, choices=["topk_mean", "max"], help="scene score aggregation")
    _parser.add_argument("--scene-topk", type=int, default=None, help="top-k windows used by topk_mean scene scoring")
    ...
    if _args.active_threshold is not None:
        config.active_prob_threshold = float(_args.active_threshold)
        logger.info(f"active_prob_threshold: {config.active_prob_threshold}")
    if _args.scene_threshold is not None:
        config.scene_active_threshold = float(_args.scene_threshold)
        logger.info(f"scene_active_threshold: {config.scene_active_threshold}")
    if _args.scene_score_mode is not None:
        config.scene_score_mode = str(_args.scene_score_mode)
        logger.info(f"scene_score_mode: {config.scene_score_mode}")
    if _args.scene_topk is not None:
        config.scene_topk = int(_args.scene_topk)
        logger.info(f"scene_topk: {config.scene_topk}")
    ...
    if os.environ.get('EVAL_ONLY') == '1':
        ...
        last_metrics = {
            ...
            'active_f1': float(test_metrics.get('active_f1', 0.0)),
            'normal_window_fpr': float(test_metrics.get('normal_window_fpr', 0.0)),
            'scene_has_defect_acc': float(test_metrics.get('scene_has_defect_acc', 0.0)),
            'scene_recall': float(test_metrics.get('scene_recall', 0.0)),
            'scene_fpr': float(test_metrics.get('scene_fpr', 0.0)),
            'scene_f1': float(test_metrics.get('scene_f1', 0.0)),
    ...
    for epoch in range(config.num_epochs):
        ...
        logger.info(
            ...
            f"F1: {val_metrics.get('active_f1', 0.0):.4f}, "
            f"NormalFPR: {val_metrics.get('normal_window_fpr', 0.0):.4f}"
        )
        ...
        logger.info(
            f"  验证 场景报警 - Acc: {val_metrics.get('scene_has_defect_acc', 0.0):.4f}, "
            f"Recall: {val_metrics.get('scene_recall', 0.0):.4f}, "
            f"FPR: {val_metrics.get('scene_fpr', 0.0):.4f}, "
            f"F1: {val_metrics.get('scene_f1', 0.0):.4f}"
        )
    ...
    logger.info(
        ...
        f"F1: {test_metrics.get('active_f1', 0.0):.4f}, "
        f"NormalFPR: {test_metrics.get('normal_window_fpr', 0.0):.4f}"
    )
    ...
    logger.info(
        "  Scene alarm - Acc: %.4f, Recall: %.4f, FPR: %.4f, F1: %.4f",
        float(test_metrics.get('scene_has_defect_acc', 0.0)),
        float(test_metrics.get('scene_recall', 0.0)),
