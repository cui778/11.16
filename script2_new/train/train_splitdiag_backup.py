...
def main():
    ...
    # 按 --run-name 备份模型、CSV、指标
    if _args.run_name:
        import shutil
        rn = _args.run_name
        ckpt = Path(config.output_dir)
        _backups = [
            (ckpt / "best_model.pth", ckpt / f"best_model_{rn}.pth"),
            (ckpt / "by_scenario_test_latest.csv", ckpt / f"by_scenario_test_{rn}.csv"),
            (ckpt / "by_scenario_val_best.csv", ckpt / f"by_scenario_val_{rn}.csv"),
        ]
        for src, dst in _backups:
            if src.exists():
                shutil.copy2(src, dst)
                logger.info(f"  [备份] {src.name} -> {dst.name}")
        # 指标也备份一份
        metrics_src = out_dir / 'last_run_metrics.json'
        metrics_dst = out_dir / f'last_run_metrics_{rn}.json'
        shutil.copy2(metrics_src, metrics_dst)
        logger.info(f"  [备份] last_run_metrics.json -> last_run_metrics_{rn}.json")
