...
def _evaluate(masked_student: nn.Module, loader: DataLoader, device: torch.device, train_seen_node_indices: set):
...
def main():
    ...
    default_output_tag = (
        ...
        f"{args.teacher_subdir}_{student_cfg.dataset_split_mode}_{time_tag}_{trend_tag}_{kd_tag}_{active_kd_tag}"
    ...
    for epoch in range(1, student_cfg.num_epochs + 1):
        ...
        val_metrics = _evaluate(masked_student, student_val, device, seen_set)
        ...
        logger.info(
            "Epoch %d/%d loss=%.4f val_mrr=%.4f val_top1=%.4f val_top3=%.4f val_top5=%.4f",
            epoch,
    ...
    masked_student = MaskedInputModel(student_model)
    test_metrics = _evaluate(masked_student, student_test, device, seen_set)
