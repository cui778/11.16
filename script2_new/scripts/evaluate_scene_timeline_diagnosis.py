...
def _features_for_set(feature_set: str) -> list[str]:
    mode = str(feature_set or "raw_plus_residual").strip().lower()
    if mode == "raw_only":
        return list(RAW4)
...
def _mask_unobserved_features(x: torch.Tensor, observed_mask: torch.Tensor) -> torch.Tensor:
    ...
    # Keep standalone inference identical to MaskedInputModel in
    # train_privileged_teacher_student.py: every dynamic channel is hidden at
    # unobserved nodes, including the appended observed-mask feature.
    return x * mask.float()
...
def _build_config(args: argparse.Namespace) -> Config:
    ...
    manifest_path = Path(cfg.training_data_dir) / args.subdir / "dataset_manifest.json"
    if manifest_path.exists():
        with manifest_path.open("r", encoding="utf-8") as handle:
            manifest = json.load(handle)
        defect_csv = str(manifest.get("defect_csv_path", "")).strip()
        if defect_csv:
            cfg.defect_matrix_file = os.path.normpath(defect_csv)
    cfg.monitor_nodes_file = os.path.normpath(args.monitor_nodes)
    cfg.selected_features = _features_for_set(args.feature_set)
    ...
    cfg.window_stride = int(args.window_stride)
...
def main() -> None:
    ...
    parser.add_argument("--subdir", default="ie420_plus_normal20_v1")
    ...
    parser.add_argument("--monitor-nodes", default=str(SCRIPT_ROOT / "chapter5_layout_optimization" / "outputs" / "layouts" / "degree" / "monitor_nodes_degree_N25.json"))
    parser.add_argument("--model-type", default="hydraulic_inverse_deepattn")
    parser.add_argument("--feature-set", default="raw_plus_residual", choices=["raw_only", "residual_only", "raw_plus_residual"])
    ...
    for sc, rows in grouped_scores.items():
        ...
        true_active_scores = np.zeros(len(node_list), dtype=np.float64)
        true_active_weight = 0.0
        target_indices = [
            int(r["target_node_idx"])
            for r in rows
            if int(r["target_node_idx"]) >= 0
        ]
        event_target_idx = target_indices[0] if target_indices else -1
        ...
        if aggregate_weight > 0:
            ...
            pred_rank = _rank_from_scores(aggregate_scores, event_target_idx)
        ...
        for r in rows:
            if int(r["true_active"]) > 0:
                # Match utils/evaluation.py event_level_topK: active-window
                # candidate probabilities are aggregated with an equal mean.
                true_active_scores += r["candidate_probs"]
                true_active_weight += 1.0
        if true_active_weight > 0:
            true_active_scores /= true_active_weight
            true_active_rank = _rank_from_scores(
                true_active_scores, event_target_idx
            )
            true_active_top_node = node_list[int(np.argmax(true_active_scores))]
        else:
            true_active_rank = None
            true_active_top_node = ""
        ...
        event_rows.append(
            {
                ...
                "true_active_top_node_id": true_active_top_node,
                "true_active_true_node_rank": "" if true_active_rank is None else int(true_active_rank),
                "n_windows": len(rows),
    ...
    metrics = {
        ...
        "onset_accuracy_within_1_stride": float((pd.to_numeric(detected_defects["onset_error_hours"], errors="coerce") <= stride_hours).mean()) if len(detected_defects) else math.nan,
        "onset_accuracy_within_2_strides": float((pd.to_numeric(detected_defects["onset_error_hours"], errors="coerce") <= 2 * stride_hours).mean()) if len(detected_defects) else math.nan,
        "onset_accuracy_within_3_strides": float((pd.to_numeric(detected_defects["onset_error_hours"], errors="coerce") <= 3 * stride_hours).mean()) if len(detected_defects) else math.nan,
    ...
    out_dir = Path(args.output_dir) if str(args.output_dir).strip() else Path(cfg.reports_dir)
