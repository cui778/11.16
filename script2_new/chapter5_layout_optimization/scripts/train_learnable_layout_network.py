...
def quality_score(row: pd.Series, preset: str) -> float:
    # 使用验证集指标，杜绝用 test 表现生成布局
    val_mrr = float(row.get("val_mrr", 0.0) or 0.0)
    val_top1 = float(row.get("val_top1", 0.0) or 0.0)
    if val_mrr <= 0.0 and val_top1 <= 0.0:
        raise ValueError("quality_score requires val_mrr and val_top1 in quality dataset")

    nh_mrr = float(row.get("node_holdout_mrr", 0.0) or 0.0)
    nh_top1 = float(row.get("node_holdout_top1", 0.0) or 0.0)

    if preset == "scenario":
        return 0.70 * val_mrr + 0.30 * val_top1
    if preset == "generalization":
        return 0.25 * val_mrr + 0.15 * val_top1 + 0.35 * nh_mrr + 0.25 * nh_top1
    return 0.45 * val_mrr + 0.20 * val_top1 + 0.20 * nh_mrr + 0.15 * nh_top1


def build_node_targets(features_df: pd.DataFrame, preset: str) -> Tuple[np.ndarray, pd.DataFrame]:
    quality_df = pd.read_csv(DATASET_PATH)
